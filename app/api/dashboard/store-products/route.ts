...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ error: "فشل جلب منتجات المكتبة" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const contentType = body.contentType === "article" ? "article" : "file";
  const pdfUrl = String(body.pdfUrl ?? "").trim();
  const articleBody = String(body.articleBody ?? "").trim();
  if (contentType === "file" && !pdfUrl) {
    return NextResponse.json({ error: "رابط ملف PDF إجباري للملفات" }, { status: 400 });
  }
  if (contentType === "article" && !articleBody) {
    return NextResponse.json({ error: "محتوى المقال مطلوب" }, { status: 400 });
  }
