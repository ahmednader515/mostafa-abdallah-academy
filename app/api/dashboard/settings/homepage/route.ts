...
export async function PUT(request: NextRequest) {
  ...
  let hero_slides_json: string | null | undefined;
  if (body.heroSlides !== undefined) {
    if (body.heroSlides === null) {
      hero_slides_json = null;
    } else if (!Array.isArray(body.heroSlides)) {
      return NextResponse.json({ error: "بيانات شرائح الهيرو غير صالحة" }, { status: 400 });
    } else {
      if (body.heroSlides.length > 8) {
        return NextResponse.json({ error: "الحد الأقصى لشرائح الهيرو هو 8 شرائح" }, { status: 400 });
      }
      const slides: AcademyHeroSlide[] = [];
      for (let i = 0; i < body.heroSlides.length; i++) {
        const parsed = parseHeroSlide(body.heroSlides[i]);
        if (!parsed) {
          return NextResponse.json(
            { error: `صورة شريحة الهيرو رقم ${i + 1} مطلوبة` },
            { status: 400 },
          );
        }
        slides.push(parsed);
      }
      hero_slides_json = serializeHeroSlides(slides);
    }
  }

  let stats_ribbon_json: string | null | undefined;
  if (body.statsRibbon !== undefined) {
    if (body.statsRibbon === null) {
      stats_ribbon_json = null;
    } else if (!Array.isArray(body.statsRibbon)) {
      return NextResponse.json({ error: "بيانات شريط الإحصائيات غير صالحة" }, { status: 400 });
    } else {
      if (body.statsRibbon.length > 4) {
        return NextResponse.json({ error: "الحد الأقصى لعناصر شريط الإحصائيات هو 4" }, { status: 400 });
      }
      const items: HomepageStatItem[] = [];
      for (let i = 0; i < body.statsRibbon.length; i++) {
        const parsed = parseHomepageStatItem(body.statsRibbon[i]);
        if (!parsed) {
          return NextResponse.json(
            { error: `عنصر شريط الإحصائيات رقم ${i + 1} غير صالح` },
            { status: 400 },
          );
        }
        items.push(parsed);
      }
      stats_ribbon_json = serializeStatsRibbon(items);
    }
  }

  let main_nav_flags_json: string | null | undefined;
  if (body.mainNavFlags !== undefined) {
    if (body.mainNavFlags === null) {
      main_nav_flags_json = null;
    } else if (typeof body.mainNavFlags !== "object") {
