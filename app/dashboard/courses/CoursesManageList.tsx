          ...
          <table className="w-full" dir={dir}>
            <thead>
              <tr className="border-b border-[var(--color-border)] bg-[var(--color-background)]/30">
                <th className={thClass}>{t(`${L}.colOrder`)}</th>
      ...
      {filteredCourses.length === 0 ? (
        ...
        <div className="space-y-8">
          {byCategory.map(({ key, title, courses: sectionCourses }) => {
            const sectionCourseIds = sectionCourses.map((c) => c.id);
            return (
              <div
                key={key}
              ...
              >
                ...
                <table className="w-full" dir={dir}>
                  ...
                  <tbody>
                    {sectionCourses.map((c, courseIdx) => (
                      <CourseTableRow
                        key={c.id}
                        c={c}
