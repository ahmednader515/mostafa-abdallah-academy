...
type LessonAttachmentRow = { title: string; fileUrl: string; fileType: string; fileName: string };
type LessonRow = {
  title: string;
  videoUrl: string;
  content: string;
...
export function CreateCourseForm() {
  ...
  const [form, setForm] = useState({
    ...
    categoryNameAr: "",
    categoryNameEn: "",
    accessType: "lifetime" as "lifetime" | "duration_days" | "subscription_only",
    accessDurationDays: "",
  ...
  const [quizzes, setQuizzes] = useState<QuizRow[]>([{ title: "", timeLimitMinutes: "", passingScore: "", questions: [{ type: "MULTIPLE_CHOICE", questionText: "", options: [{ text: "", isCorrect: false }] }] }]);
  const [contentOrder, setContentOrder] = useState<ContentOrderEntry[]>([{ type: "lesson", index: 0 }, { type: "quiz", index: 0 }]);
  const [imageUploading, setImageUploading] = useState(false);
  const [imageUploadError, setImageUploadError] = useState("");
  const [fileUploading, setFileUploading] = useState<{ lesson: number; attachment: number } | null>(null);

  function syncPdfUrl(attachments: LessonAttachmentRow[]): string {
    return attachments[0]?.fileUrl?.trim() || "";
  }
  ...
  function updateLesson(i: number, field: keyof LessonRow, value: string | boolean | LessonAttachmentRow[]) {
    setLessons((l) => l.map((x, idx) => (idx === i ? { ...x, [field]: value } : x)));
  }

  function addAttachment(lessonIdx: number) {
    setLessons((l) =>
      l.map((x, idx) =>
        idx === lessonIdx
          ? { ...x, attachments: [...x.attachments, { title: "", fileUrl: "", fileType: "other", fileName: "" }] }
          : x
      )
    );
  }

  function removeAttachment(lessonIdx: number, attachmentIdx: number) {
    setLessons((l) =>
      l.map((x, idx) => {
        if (idx !== lessonIdx) return x;
        const attachments = x.attachments.filter((_, j) => j !== attachmentIdx);
        return { ...x, attachments, pdfUrl: syncPdfUrl(attachments) };
      })
    );
  }

  function updateAttachment(lessonIdx: number, attachmentIdx: number, field: keyof LessonAttachmentRow, value: string) {
    setLessons((l) =>
      l.map((x, idx) => {
        if (idx !== lessonIdx) return x;
        const attachments = x.attachments.map((a, j) => (j === attachmentIdx ? { ...a, [field]: value } : a));
        return { ...x, attachments, pdfUrl: syncPdfUrl(attachments) };
      })
    );
  }
  ...
  function setAttachmentFile(
    ...
    setLessons((l) =>
      l.map((x, idx) => {
        if (idx !== lessonIdx) return x;
        const attachments = x.attachments.map((a, j) =>
          j === attachmentIdx ? { ...a, fileUrl, fileType, fileName: fileName || a.fileName } : a
        );
        return { ...x, attachments, pdfUrl: syncPdfUrl(attachments) };
      })
    );
  ...
  function addQuiz() {
    setQuizzes((q) => [...q, { title: "", timeLimitMinutes: "", passingScore: "", questions: [{ type: "MULTIPLE_CHOICE", questionText: "", options: [{ text: "", isCorrect: false }] }] }]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      lessons: validLessons.map((l) => ({
          ...
          pdfUrl: syncPdfUrl(l.attachments) || l.pdfUrl.trim() || undefined,
          attachments: l.attachments
            .filter((a) => a.fileUrl.trim())
            .map((a) => ({
              title: a.title.trim(),
              fileUrl: a.fileUrl.trim(),
              fileType: a.fileType.trim() || "other",
    ...
    <form onSubmit={handleSubmit} className="mt-6 max-w-3xl space-y-8">
      ...
      <section className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--color-foreground)]">{t(`${Cf}.courseDurationLabel`)}</label>
            <input
              type="text"
              value={form.duration}
              onChange={(e) => setForm((f) => ({ ...f, duration: e.target.value }))}
              placeholder={t(`${Cf}.courseDurationPlaceholder`)}
              className="mt-1 w-full rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-[var(--color-foreground)]">{t(`${Cf}.courseLevelLabel`)}</label>
            <select
              value={form.level}
              onChange={(e) => setForm((f) => ({ ...f, level: e.target.value }))}
              className="mt-1 w-full rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2"
            >
              <option value="">{t(`${Cf}.levelNone`)}</option>
              <option value="beginner">{t(`${Cf}.levelBeginner`)}</option>
              <option value="intermediate">{t(`${Cf}.levelIntermediate`)}</option>
              <option value="advanced">{t(`${Cf}.levelAdvanced`)}</option>
            </select>
          </div>
          <div>
          ...
          <div key={i} className="mb-6 rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] p-4">
            ...
            <div className="space-y-2">
              ...
              <div>
                  ...
                  <div key={ai} className="mt-2 rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-surface)] p-3">
                    <div className="flex flex-wrap items-center gap-2">
                      <input
                        type="text"
                        value={att.title}
                        onChange={(e) => updateAttachment(i, ai, "title", e.target.value)}
                      ...
                      {isLinkKind ? (
                        ...
                        <>
                          <a href={att.fileUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-[var(--color-primary)] hover:underline">
                            {t(`${Cf}.viewPdf`)}
                          </a>
                        ...
                        <label className="inline-block cursor-pointer rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2 text-sm">
                          {fileUploading?.lesson === i && fileUploading.attachment === ai
                            ? t(`${Cf}.uploadingPdf`)
                            : t(`${Cf}.choosePdfUpload`)}
                          <input
                            type="file"
                            ...
                            onChange={async (e) => {
                              const f = e.target.files?.[0];
                              if (!f) return;
                              setFileUploading({ lesson: i, attachment: ai });
                              try {
                                const fd = new FormData();
                                fd.set("file", f);
                                ...
                                const data = await res.json().catch(() => ({}));
                                if (res.ok && data.url) {
                                  setAttachmentFile(i, ai, data.url, data.fileType || "other", f.name);
                                }
                              } finally {
                ...
                <button
                  type="button"
                  onClick={() => addAttachment(i)}
                  className="mt-2 rounded-[var(--radius-btn)] border border-[var(--color-border)] px-3 py-2 text-sm font-medium"
                >
