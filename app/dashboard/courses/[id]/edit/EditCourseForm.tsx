    ...
    initialData.lessons.length > 0
      ? initialData.lessons.map((l) => {
          ...
          const rawAttachments = Array.isArray(r.attachments)
            ? (r.attachments as Array<Record<string, unknown>>).map((a) => ({
            ...
            : [];
          const pdfUrl = String(r.pdfUrl ?? r.pdf_url ?? "");
          const attachments =
      ...
      <section className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.duration} onChange={(e) => setForm((f) => ({ ...f, duration: e.target.value }))} placeholder={t(`${Cf}.courseDurationPlaceholder`)} className="mt-1 w-full rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2" />
          </div>
          <div>
            <label className="block text-sm font-medium text-[var(--color-foreground)]">{t(`${Cf}.courseLevelLabel`)}</label>
            <select value={form.level} onChange={(e) => setForm((f) => ({ ...f, level: e.target.value }))} className="mt-1 w-full rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {att.fileUrl ? (
                        ...
                        <label className="inline-block cursor-pointer rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2 text-sm">
                          {fileUploading?.lesson === i && fileUploading.attachment === ai ? t(`${Cf}.uploadingPdf`) : t(`${Cf}.choosePdfUpload`)}
                          <input
                            type="file"
