...
import { getCoursesWithCounts, getCoursesWithCountsForCreator, getCategoriesForDashboard } from "@/lib/db";
...
export default async function DashboardCoursesPage() {
  ...
  const categoriesPlain = categories.map((cat) => {
    const row = cat as { id: string; name: string; name_ar?: string | null; nameAr?: string | null };
    return {
      id: String(row.id),
      name: String(row.name ?? ""),
      nameAr: row.nameAr ?? row.name_ar ?? null,
    };
  });
  ...
  return (
    <div>
      ...
      <CoursesManageList courses={coursesPlain} categories={categoriesPlain} />
    </div>
  );
}
