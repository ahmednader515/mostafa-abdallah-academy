...
export function LibraryAdminClient({
  initialEnabled,
  ...
  const t = useT();
  const S = "dashboard.libraryAdmin";
  ...
  const [editContentType, setEditContentType] = useState<"file" | "article">("file");
  ...
  const reloadCategories = useCallback(async () => {
    const res = await fetch("/api/dashboard/library-categories", { credentials: "include" });
    if (!res.ok) return;
    const data = (await res.json()) as { categories?: LibraryCategory[] };
  ...
  async function createProduct(e: React.FormEvent) {
    ...
    if (contentType === "file" && !pdfUrl.trim()) {
      setError(t(`${S}.pdfRequired`));
      return;
    }
    if (contentType === "article" && !articleBody.trim()) {
      setError(t(`${S}.articleBodyRequired`, "Article body is required"));
      return;
    }
    setLoading(true);
  ...
  function startEdit(row: StoreProduct) {
    ...
    setEditContentType(row.contentType === "article" ? "article" : "file");
    setEditCategoryId(row.categoryId ?? "");
    setEditArticleBody(row.articleBody ?? "");
    setEditImageError("");
  }
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    if (editContentType === "file" && !editPdfUrl.trim()) {
      setError(t(`${S}.editPdfRequired`));
      return;
    }
    if (editContentType === "article" && !editArticleBody.trim()) {
      setError(t(`${S}.articleBodyRequired`, "Article body is required"));
      return;
    }
    setLoading(true);
    const res = await fetch(`/api/dashboard/store-products/${encodeURIComponent(editingId)}`, {
      ...
      body: JSON.stringify({
        ...
        categoryId: editCategoryId.trim() || null,
        articleBody: editArticleBody.trim() || null,
      }),
    });
    const data = await res.json().catch(() => ({}));
    setLoading(false);
  ...
  async function createCategory(e: React.FormEvent) {
    ...
    if (!catName.trim() || !catSlug.trim()) {
      setError(t(`${S}.categoryValidation`, "Category name and slug are required"));
      return;
    }
    setLoading(true);
    const res = await fetch("/api/dashboard/library-categories", {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: catName.trim(),
        nameAr: catNameAr.trim() || null,
        slug: catSlug.trim(),
        parentId: catParentId.trim() || null,
      }),
    });
    const data = await res.json().catch(() => ({}));
    setLoading(false);
    if (!res.ok) return setError(data.error ?? t(`${S}.categoryCreateFailed`, "Failed to create category"));
    setSuccess(t(`${S}.categoryCreateSuccess`, "Category created"));
    setCatName("");
    setCatNameAr("");
    setCatSlug("");
    setCatParentId("");
  ...
  async function removeCategory(row: LibraryCategory) {
    if (!window.confirm(fillMessage(t(`${S}.confirmDeleteCategory`, "Delete category «{name}»?"), { name: row.name }))) return;
    const res = await fetch(`/api/dashboard/library-categories/${encodeURIComponent(row.id)}`, {
      method: "DELETE",
      credentials: "include",
    });
    if (!res.ok) return;
  ...
  return (
    <div className="space-y-8">
      ...
      <form onSubmit={(e) => void createCategory(e)} className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 space-y-3">
        <h3 className="text-lg font-semibold text-[var(--color-foreground)]">{t(`${S}.categoriesTitle`, "Library categories")}</h3>
        <div className="grid gap-3 sm:grid-cols-2">
          <input value={catName} onChange={(e) => setCatName(e.target.value)} placeholder={t(`${S}.categoryNamePlaceholder`, "Category name (EN)")} className="rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2" />
          <input value={catNameAr} onChange={(e) => setCatNameAr(e.target.value)} placeholder={t(`${S}.categoryNameArPlaceholder`, "Category name (AR)")} className="rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2" />
          <input value={catSlug} onChange={(e) => setCatSlug(e.target.value)} placeholder={t(`${S}.categorySlugPlaceholder`, "Slug")} className="rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2" />
          <select value={catParentId} onChange={(e) => setCatParentId(e.target.value)} className="rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2">
            <option value="">{t(`${S}.parentCategoryNone`, "No parent (top-level)")}</option>
            {parentCategories.map((c) => (
              <option key={c.id} value={c.id}>{c.nameAr || c.name}</option>
            ))}
          </select>
        </div>
        <button disabled={loading} className="rounded-[var(--radius-btn)] bg-[var(--color-primary)] px-4 py-2 text-sm text-white disabled:opacity-50">{t(`${S}.addCategory`, "Add category")}</button>
        <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((c, idx) => (
            <div key={c.id} className="rounded-[var(--radius-btn)] border border-[var(--color-border)] p-3 text-sm">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="font-semibold">{c.nameAr || c.name}</p>
                  <p className="text-xs text-[var(--color-muted)]">{c.slug}{c.parentId ? ` · ${t(`${S}.subcategory`, "Subcategory")}` : ""}</p>
                </div>
                <DashboardReorderButtons
                  entity="libraryCategory"
                  orderedIds={categories.map((row) => row.id)}
                  index={idx}
                  onReordered={() => {
              ...
              <button type="button" onClick={() => void removeCategory(c)} className="mt-2 text-xs text-red-500 underline">{t(`${S}.delete`)}</button>
            </div>
          ))}
          {categories.length === 0 ? <p className="text-sm text-[var(--color-muted)]">{t(`${S}.noCategories`, "No categories yet.")}</p> : null}
        </div>
      </form>
      ...
      <form onSubmit={(e) => void createProduct(e)} className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2">
            ...
            {categories.map((c) => (
              <option key={c.id} value={c.id}>{c.parentId ? "— " : ""}{c.nameAr || c.name}</option>
            ))}
          </select>
        </div>
        <div>
        ...
        {contentType === "article" ? (
          <textarea
            ...
            placeholder={t(`${S}.articleBodyPlaceholder`, "Article body")}
            rows={8}
            className="w-full rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2"
          />
        ) : null}
      ...
      <div className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
        ...
        {editingId ? (
          <form onSubmit={(e) => void saveEdit(e)} className="mt-4 space-y-3 rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <select value={editContentType} onChange={(e) => setEditContentType(e.target.value as "file" | "article")} className="rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2">
                <option value="file">{t(`${S}.contentTypeFile`, "File / PDF")}</option>
                <option value="article">{t(`${S}.contentTypeArticle`, "Article")}</option>
              </select>
              <select value={editCategoryId} onChange={(e) => setEditCategoryId(e.target.value)} className="rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2">
                <option value="">{t(`${S}.noCategory`, "No category")}</option>
                {categories.map((c) => (
                  <option key={c.id} value={c.id}>{c.parentId ? "— " : ""}{c.nameAr || c.name}</option>
                ))}
          ...
          {products.map((p, idx) => (
            <div key={p.id} className="rounded-[var(--radius-btn)] border border-[var(--color-border)] p-4">
              <div className="flex items-start justify-between gap-2">
                ...
                <DashboardReorderButtons
                  entity="libraryItem"
                  orderedIds={products.map((row) => row.id)}
                  index={idx}
                  onReordered={() => {
                    void reload();
                    router.refresh();
              ...
              <p className="mt-1 text-xs text-[var(--color-muted)]">
                {p.contentType === "article" ? t(`${S}.contentTypeArticle`, "Article") : t(`${S}.contentTypeFile`, "File / PDF")}
              </p>
