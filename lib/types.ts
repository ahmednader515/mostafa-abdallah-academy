...
export interface HomepageSetting {
  ...
  /** شرائح الهيرو الحالية (JSON: AcademyHeroSlide[]) — إن فارغ تُستخدم القيم الافتراضية من الكود */
  heroSlidesJson?: string | null;
  /** شريط الإحصائيات تحت الهيرو (JSON: HomepageStatItem[]) */
...
export interface LessonAttachment {
  ...
  lesson_id: string;
  title: string;
  file_type: LessonAttachmentFileType | string;
  file_url: string;
  file_name: string | null;
  order: number;
  created_at?: Date;
...
export interface LessonAttachmentApp {
  title: string;
  fileUrl: string;
  fileType: string;
...
export interface LessonApp {
  ...
  pdfUrl?: string | null;
  attachments?: LessonAttachmentApp[];
  duration?: number | null;
  order: number;
  courseId: string;
  createdAt?: Date;
