...
async function ensureHomepageHeroStatsNavColumns(): Promise<void> {
  return ensureOnce("ensureHomepageHeroStatsNavColumns", async () => {
    try {
...
async function getHomepageSettingsUncached(): Promise<HomepageSetting> {
  try {
    ...
    return {
      ...
      heroSlidesJson: (() => {
        const raw = row.hero_slides_json ?? (c as { heroSlidesJson?: unknown }).heroSlidesJson;
        const s = raw != null ? String(raw).trim() : "";
        if (s.length > 0) return s.slice(0, 50000);
        return HOMEPAGE_DEFAULTS.heroSlidesJson ?? null;
      })(),
      statsRibbonJson: (() => {
        const raw = row.stats_ribbon_json ?? (c as { statsRibbonJson?: unknown }).statsRibbonJson;
        const s = raw != null ? String(raw).trim() : "";
        if (s.length > 0) return s.slice(0, 8000);
        return HOMEPAGE_DEFAULTS.statsRibbonJson ?? null;
      })(),
      mainNavFlagsJson: (() => {
        const raw =
          row.main_nav_flags_json ?? (c as { mainNavFlagsJson?: unknown }).mainNavFlagsJson;
        const s = raw != null ? String(raw).trim() : "";
        if (s.length > 0) return s.slice(0, 2000);
...
export async function updateHomepageSettings(data: {
  ...
  if (data.hero_slides_json !== undefined) {
    await ensureHomepageHeroStatsNavColumns();
    await sql`UPDATE "HomepageSetting" SET hero_slides_json = ${data.hero_slides_json}, updated_at = NOW() WHERE id = 'default'`;
  }
  if (data.stats_ribbon_json !== undefined) {
    await ensureHomepageHeroStatsNavColumns();
    await sql`UPDATE "HomepageSetting" SET stats_ribbon_json = ${data.stats_ribbon_json}, updated_at = NOW() WHERE id = 'default'`;
  }
  if (data.main_nav_flags_json !== undefined) {
    await ensureHomepageHeroStatsNavColumns();
...
function mapStoreProduct(r: Record<string, unknown>): StoreProductRow {
  const contentType = String(r.content_type ?? "file");
  return {
    id: String(r.id),
...
export async function getStoreProductById(id: string): Promise<StoreProductRow | null> {
  await ensureStoreProductsSchema();
  const pid = id.trim();
  if (!pid) return null;
  const rows = await sql`
...
export async function createStoreProduct(data: {
  ...
  const contentType = data.content_type === "article" ? "article" : "file";
  await sql`
...
export async function updateStoreProduct(
  ...
  if (data.category_id !== undefined) await sql`UPDATE "StoreProduct" SET category_id = ${data.category_id?.trim() || null}, updated_at = NOW() WHERE id = ${pid}`;
  if (data.content_type !== undefined) {
    const ct = data.content_type === "article" ? "article" : "file";
    await sql`UPDATE "StoreProduct" SET content_type = ${ct}, updated_at = NOW() WHERE id = ${pid}`;
  }
  if (data.article_body !== undefined) await sql`UPDATE "StoreProduct" SET article_body = ${data.article_body?.trim() || null}, updated_at = NOW() WHERE id = ${pid}`;
}
...
export type CoursePublishedExtras = {
...
export async function getCoursesPublished(
  ...
  return (rows as Record<string, unknown>[]).map((r) => {
    const category = r.cat_id
      ? rowToCamel({ id: r.cat_id, name: r.cat_name, name_ar: r.cat_name_ar, slug: r.cat_slug })
      : null;
    const { cat_id, cat_name, cat_name_ar, cat_slug, instructor_name, lessons_count, ...rest } = r;
    const base = rowToCamel(rest) ?? {};
    return {
      ...base,
      category,
      lessonsCount: Number(lessons_count ?? 0),
      instructorName: instructor_name != null ? String(instructor_name) : null,
    };
  }) as unknown as (Course & CoursePublishedExtras)[];
}
...
export async function createCourse(data: {
  ...
  try {
    rows = await sql`
      INSERT INTO "Course" (id, title, title_ar, slug, description, description_en, short_desc, short_desc_en, image_url, price, duration, level, is_published, created_by_id, max_quiz_attempts, category_id, accepts_homework)
      VALUES (${id}, ${data.title}, ${data.title_ar}, ${data.slug}, ${data.description}, ${data.description_en ?? null}, ${data.short_desc ?? null}, ${data.short_desc_en ?? null}, ${data.image_url ?? null}, ${data.price}, ${duration}, ${level}, ${data.is_published}, ${data.created_by_id}, ${data.max_quiz_attempts ?? null}, ${catId}, ${acceptsHomework})
      RETURNING *
    `;
  } catch (err) {
...
async function ensureLessonAttachmentsSchema(): Promise<void> {
  return ensureOnce("ensureLessonAttachmentsSchema", async () => {
    await sql`
    ...
    `;
    await sql`CREATE INDEX IF NOT EXISTS "LessonAttachment_lesson_id_idx" ON "LessonAttachment"(lesson_id, "order")`;
  });
}

function attachmentRowToApp(row: Record<string, unknown>): {
  title: string;
  fileUrl: string;
  fileType: string;
  ...
  return {
    ...
    fileUrl: String(camel.fileUrl ?? ""),
    fileType: String(camel.fileType ?? "other"),
    fileName: camel.fileName != null ? String(camel.fileName) : null,
  };
}

function legacyAttachmentFromPdfUrl(pdfUrl: unknown): {
  title: string;
  fileUrl: string;
  fileType: string;
  fileName: string | null;
} | null {
  const url = pdfUrl != null ? String(pdfUrl).trim() : "";
  if (!url) return null;
  return { title: "", fileUrl: url, fileType: "pdf", fileName: null };
}

async function enrichLessonsWithAttachments(lessonRows: Record<string, unknown>[]): Promise<Record<string, unknown>[]> {
  await ensureLessonAttachmentsSchema();
  if (lessonRows.length === 0) return [];

  const lessonIds = lessonRows.map((l) => String(l.id));
  const attachmentRows = await sql`
    SELECT * FROM "LessonAttachment"
  ...
  for (const row of attachmentRows as Record<string, unknown>[]) {
    ...
    byLesson.get(lessonId)!.push(row);
  }

  return lessonRows.map((lesson) => {
    const lessonId = String(lesson.id);
    const stored = byLesson.get(lessonId) ?? [];
    const attachments =
      stored.length > 0
        ? stored.map(attachmentRowToApp)
        : (() => {
            const legacy = legacyAttachmentFromPdfUrl(lesson.pdfUrl ?? lesson.pdf_url);
...
export async function getLessonAttachments(lessonId: string): Promise<LessonAttachment[]> {
  ...
  const rows = await sql`
    ...
    ORDER BY "order" ASC
  `;
  if (rows.length > 0) return rows as LessonAttachment[];

  const lesson = await getLessonById(lessonId);
  const legacy = legacyAttachmentFromPdfUrl(lesson?.pdf_url);
  if (!legacy) return [];
  return [
    {
      id: `legacy-${lessonId}`,
      lesson_id: lessonId,
      title: legacy.title,
      file_type: legacy.fileType,
      file_url: legacy.fileUrl,
...
export async function createLessonAttachment(data: {
  ...
  const id = generateId();
  await sql`
    INSERT INTO "LessonAttachment" (id, lesson_id, title, file_type, file_url, file_name, "order")
    VALUES (
      ${id},
      ${data.lesson_id},
      ${data.title ?? ""},
  ...
  `;
  const rows = await sql`SELECT * FROM "LessonAttachment" WHERE id = ${id} LIMIT 1`;
  const row = rows[0] as LessonAttachment;
  if (!row) throw new Error("فشل إنشاء مرفق الحصة");
  return row;
}

export async function replaceLessonAttachments(
  lessonId: string,
  items: Array<{
    title?: string;
    file_type?: string;
  ...
  await ensureLessonAttachmentsSchema();
  await sql`DELETE FROM "LessonAttachment" WHERE lesson_id = ${lessonId}`;
  let order = 0;
...
export async function persistLessonAttachmentsFromInput(
  lessonId: string,
  input: {
    pdfUrl?: string | null;
    attachments?: Array<{ title?: string; fileUrl?: string; fileType?: string; fileName?: string | null }>;
  }
): Promise<void> {
  const items =
    input.attachments
      ?.map((a) => ({
        title: a.title?.trim() || "",
        file_type: a.fileType?.trim() || "other",
  ...
  if (items.length > 0) {
    await replaceLessonAttachments(lessonId, items);
    return;
  }

  const pdfUrl = input.pdfUrl?.trim();
  if (pdfUrl) {
    await replaceLessonAttachments(lessonId, [
      { title: "", file_type: "pdf", file_url: pdfUrl, file_name: null, order: 0 },
    ]);
  } else {
    await replaceLessonAttachments(lessonId, []);
  }
...
export async function getCourseWithContent(segment: string): Promise<{
  ...
  `;
  const lessons = await enrichLessonsWithAttachments(rowsToCamel(lessonRows as Record<string, unknown>[]));
...
let libraryCategorySchemaEnsured = false;

async function ensureLibraryCategorySchema(): Promise<void> {
  if (libraryCategorySchemaEnsured) return;
  try {
    await sql`
    ...
    await ensureStoreProductsSchema();
    await sql`ALTER TABLE "StoreProduct" ADD COLUMN IF NOT EXISTS category_id TEXT`;
    libraryCategorySchemaEnsured = true;
  } catch {
    /* DDL غير متاح */
  }
}
...
function mapLibraryCategory(r: Record<string, unknown>): LibraryCategoryRow {
  return {
    id: String(r.id),
    name: String(r.name ?? ""),
...
export async function listLibraryCategoriesAll(): Promise<LibraryCategoryRow[]> {
  try {
    ...
    `;
    return (rows as Record<string, unknown>[]).map(mapLibraryCategory);
  } catch {
    return [];
  }
}

export async function getLibraryCategoryById(id: string): Promise<LibraryCategoryRow | null> {
  await ensureLibraryCategorySchema();
  const cid = id.trim();
  if (!cid) return null;
  const rows = await sql`
...
export async function createLibraryCategory(data: {
  ...
  const id = generateId();
  await sql`
    INSERT INTO "LibraryCategory" (id, name, name_ar, slug, parent_id, "order")
    VALUES (
      ${id},
      ${data.name.trim()},
...
export async function updateLibraryCategory(
  ...
  await ensureLibraryCategorySchema();
  const cid = id.trim();
  if (!cid) throw new Error("معرّف التصنيف مطلوب");
  if (data.name !== undefined) await sql`UPDATE "LibraryCategory" SET name = ${data.name.trim()} WHERE id = ${cid}`;
  if (data.name_ar !== undefined) await sql`UPDATE "LibraryCategory" SET name_ar = ${data.name_ar?.trim() || null} WHERE id = ${cid}`;
  if (data.slug !== undefined) await sql`UPDATE "LibraryCategory" SET slug = ${data.slug.trim()} WHERE id = ${cid}`;
  if (data.parent_id !== undefined) await sql`UPDATE "LibraryCategory" SET parent_id = ${data.parent_id?.trim() || null} WHERE id = ${cid}`;
  if (data.order !== undefined) await sql`UPDATE "LibraryCategory" SET "order" = ${data.order} WHERE id = ${cid}`;
}

export async function deleteLibraryCategory(id: string): Promise<void> {
  await ensureLibraryCategorySchema();
  await sql`DELETE FROM "LibraryCategory" WHERE id = ${id.trim()}`;
}

// ─── Job postings ───────────────────────────────────────────────────────────

let jobPostingSchemaEnsured = false;

async function ensureJobPostingSchema(): Promise<void> {
...
export async function getJobById(id: string): Promise<JobPostingRow | null> {
  await ensureJobPostingSchema();
  const jid = id.trim();
  if (!jid) return null;
  const rows = await sql`
...
export async function updateJob(
  ...
  await ensureJobPostingSchema();
  const jid = id.trim();
  if (!jid) throw new Error("معرّف الوظيفة مطلوب");
  if (data.title !== undefined) await sql`UPDATE "JobPosting" SET title = ${data.title.trim()}, updated_at = NOW() WHERE id = ${jid}`;
  if (data.title_ar !== undefined) await sql`UPDATE "JobPosting" SET title_ar = ${data.title_ar?.trim() || null}, updated_at = NOW() WHERE id = ${jid}`;
  if (data.description !== undefined) await sql`UPDATE "JobPosting" SET description = ${data.description.trim()}, updated_at = NOW() WHERE id = ${jid}`;
...
export async function deleteJob(id: string): Promise<void> {
  await ensureJobPostingSchema();
  await sql`DELETE FROM "JobPosting" WHERE id = ${id.trim()}`;
}

export async function reorderJobs(id: string, direction: "up" | "down"): Promise<void> {
  await ensureJobPostingSchema();
  const jid = id.trim();
  if (!jid) throw new Error("معرّف الوظيفة مطلوب");
  const current = await getJobById(jid);
  if (!current) throw new Error("الوظيفة غير موجودة");

  const all = await listAllJobs();
  const idx = all.findIndex((j) => j.id === jid);
  if (idx < 0) return;
  ...
  await sql`UPDATE "JobPosting" SET "order" = ${otherOrder}, updated_at = NOW() WHERE id = ${jid}`;
  await sql`UPDATE "JobPosting" SET "order" = ${currentOrder}, updated_at = NOW() WHERE id = ${other.id}`;
}
...
export async function updateCategoryOrder(ids: string[]): Promise<void> {
  await ensureCategoryCreatedByColumn();
  for (let i = 0; i < ids.length; i++) {
    const id = ids[i]?.trim();
    if (!id) continue;
    await sql`UPDATE "Category" SET "order" = ${i}, updated_at = NOW() WHERE id = ${id}`;
  }
}

export async function updateCourseOrder(ids: string[]): Promise<void> {
  for (let i = 0; i < ids.length; i++) {
    const id = ids[i]?.trim();
    if (!id) continue;
    await sql`UPDATE "Course" SET "order" = ${i}, updated_at = NOW() WHERE id = ${id}`;
  }
}

export async function updateLibraryCategoryOrder(ids: string[]): Promise<void> {
  await ensureLibraryCategorySchema();
  for (let i = 0; i < ids.length; i++) {
    const id = ids[i]?.trim();
    if (!id) continue;
    await sql`UPDATE "LibraryCategory" SET "order" = ${i} WHERE id = ${id}`;
  }
}

export async function updateLibraryItemOrder(ids: string[]): Promise<void> {
  await ensureStoreProductsSchema();
  for (let i = 0; i < ids.length; i++) {
    const id = ids[i]?.trim();
    if (!id) continue;
    await sql`UPDATE "StoreProduct" SET sort_order = ${i}, updated_at = NOW() WHERE id = ${id}`;
  }
}

export async function updateJobOrder(ids: string[]): Promise<void> {
  await ensureJobPostingSchema();
  for (let i = 0; i < ids.length; i++) {
    const id = ids[i]?.trim();
    if (!id) continue;
    await sql`UPDATE "JobPosting" SET "order" = ${i}, updated_at = NOW() WHERE id = ${id}`;
  }
}
